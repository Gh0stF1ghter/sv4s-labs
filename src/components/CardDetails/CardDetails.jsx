    ...
    <div className="fixed top-0 left-0 z-40 flex items-center justify-center w-full h-full backdrop-blur-md">
      <div className="glassmorphism w-[50vw]">
        ...
        <TableContainer>
          ...
          >
            <TableHead>
              <TableRow className="text-xl font-semibold capitalize">
                {value.manufacturer} {value.name}
              </TableRow>
            </TableHead>
            <Divider orientation="horizontal" />
            <TableBody className="flex flex-wrap gap-4 mt-3 ">
              {details.map(([key, value]) => (
                <TableRow
                  className="flex justify-between w-full gap-5 text-right "
                  key={key}
                >
                  <TableCell align="left" className="capitalize text-grey">
                    {key.split("_").join(" ")}
                  </TableCell>
                  <TableCell className="font-semibold text-black-100">
                    {value}
                  </TableCell>
                </TableRow>
              ))}
